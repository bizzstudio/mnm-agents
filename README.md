...
מחירי הצעת המחיר נקובים **לפני מע"מ**, והמסמך מציג: סה"כ לפני מע"מ, מע"מ,
ובמידת הצורך "מזה פטור ממע"מ", וסה"כ לתשלום. האחוז מגיע מ-
`utils/priceUtils.getCatalogVatPercent` (`VAT_PERCENTAGE`, ברירת מחדל 18) —
אותו מקור שמשמש את ריווחית ואת תמחור הקטלוג.

**פטור ממע"מ (פירות וירקות).** המע"מ מחושב **לפי שורה**: שורה שסומנה
`isVatFree: true` אינה נושאת מע"מ. הפטור מנוהל בקטלוג הראשי בלבד (ייבוא
המוצרים מסמן פירות/ירקות אוטומטית) — `AgentProduct` אינו מגדיר אותו, ולכן
`buildServerCart` שולף אותו מ-`Product` לפי ברקוד ושומר בשורת ההצעה.
**מוצר שאין לו התאמה בקטלוג הראשי נחשב חייב במע"מ**; ערך חסר לעולם אינו
נחשב פטור, אחרת ברירת המחדל בסכימה (`true`) הייתה מאפסת מע"מ בכל ההצעות.

`order.total` שנשמר במסד הוא הסכום **לפני** מע"מ. החישוב תצוגתי בלבד ואינו
נכתב למסד, כדי שדוחות, יעדי סוכנים וסכומים מצטברים ימשיכו לעבוד כמו קודם.

**המרה להזמנה.** `orderController.updateOrder` הופך הצעה להזמנה כשמשנים לה
סטטוס (למעט `Quote` ו-`Cancel`), ובאותה פעולה **מכפיל במע"מ את מחירי השורות
החייבות בלבד** — כי בהזמנת חנות המחיר השמור נחשב כולל מע"מ, ואילו בהצעה הוא
נקוב לפניו. בלי זה הצעה שאושרה על 227 ₪ הייתה מחויבת כ-200 ₪ כולל מע"מ.

ההמרה מוגנת בתנאי `orderType: "quote"` בשאילתה עצמה (שתי לחיצות במקביל לא
יכפילו פעמיים), מתועדת ב-`quote.convertedAt` / `vatAppliedPercent` /
`totalBeforeVat` וברשומת יומן `converted`, ובכל שורה נשמר `priceBeforeVat`.
כן נבנה בשורה `finalPriceAtPurchase` — ריווחית ותבניות המייל מצפות לו,
ובלעדיו הנפקת חשבונית על הזמנה שמקורה בהצעה הייתה קורסת.

את ההמרה מפעילה ההנהלה בכפתור ייעודי בעמוד ההזמנה באדמין
(`mnm-admin/src/components/order/ConvertQuoteToOrder.jsx`): חלונית אישור
שמציגה את הסכומים שהלקוח אישר, מזהירה אם ההצעה טרם אושרה, ומעדכנת את הסטטוס
ל-`Processing` — אותו סטטוס שאליו מגיעה הזמנת חנות אחרי תשלום. לסוכן אין
גישה לפעולה (הקצה מוגן ב-`isAdmin`, וקצה `convert-to-order` באפליקציית
הסוכנים מחזיר 403).

> הערה: אחרי ההמרה `orderType` כבר אינו `"quote"`, ולכן קישור הצפייה של הלקוח
> ומסמך ההצעה בצד הסוכן מפסיקים לעבוד — זו ההתנהגות שנדרשה.

### שם הספק

שם הספק (`supplier`, מיובא מעמודת "שם ספק" באקסל) מוצג **להנהלה בלבד**, בעמוד
ההזמנה באדמין מתחת לשם המוצר, כדי לדעת ממי להזמין כשאין מלאי. הוא אינו נשמר
בשורת ההזמנה אלא נשלף ב-`getOrderById` רק כשהבקשה היא של אדמין
(`req.user.role ∈ {Admin, CEO}` — טוקן לקוח ומלקט אינם נושאים role), כך שקצה
הלקוח וקצה הליקוט לא מקבלים אותו גם בעתיד. `projectProductForAgent` מוחק אותו
מתשובת הקטלוג לסוכן.

### ברקודים ולוגו במסמך

הברקודים הוסרו מהמסמך ומהתצוגה של הלקוח (הם נשמרים במסד ומוצגים לאדמין).
הלוגו מוטמע כ-data URI מ-`mnm-backend/assets/quote-logo.png` (עותק של
`mnm-admin/public/logo.png`) — לא כתובת חיצונית, כדי שיוצג גם ב-PDF שנפתח
מחוץ לרשת. אם הקובץ חסר בפריסה, המסמך נופל חזרה לכותרת טקסט ומדפיס אזהרה.
כתובת המייל שמוצגת ללקוח היא `COMPANY_EMAIL` (ברירת מחדל `office@nmplus.co.il`)
ואינה כתובת השליחה (`EMAIL_USER`).

### סרגלי מחיר באדמין

`mnm-admin/src/components/order/PriceScaleBar.jsx` מציג בעמוד ההזמנה של האדמין
את הטווח שהותר לסוכן ואת המחיר שבחר, לכל שורה בהצעה. מוסתר בהדפסה
(`print:hidden`) כי זה מידע פנימי. הרכיב משוכפל בכוונה מ-`PriceScale` של
אפליקציית הסוכנים — שני פרויקטים נפרדים בלי קוד משותף.
