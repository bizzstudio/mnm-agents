...
סוכן יכול להתחבר, לבחור לקוח קיים (או ליצור חדש אם יש לו הרשאה), לדפדף בקטלוג עם המחירים המותאמים לסוכן או ללקוח, לבנות סל, לתת הנחה ברמת שורה או הזמנה (בתוך הטווח שמוגדר עליו), לשמור כהצעת מחיר או לאשר כהזמנה, ולעקוב אחר ביצועיו מול יעדים.
...
5. **סיכום סל** (`/cart`) — עריכת כמויות, הנחה פר-שורה, הנחה כללית, הערה. שני כפתורים: "שמור כהצעת מחיר" / "אישור הזמנה".
6. **אישור** (`/confirmation/:id`) — מסך הצלחה עם מספר חשבונית.
7. **היסטוריה** (`/orders`) — רשימת הזמנות והצעות של הסוכן בלבד. הצעת מחיר ניתנת להמרה להזמנה דרך מסך הפרטים.
...
7. **בחר "שמור כהצעת מחיר"** → מסך אישור → עבור ל־`/orders` → "המר להזמנה".
